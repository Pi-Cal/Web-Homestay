...
export const EditAccount = ({ account, isGetting }) => {
      ...
      await updateInfo(userState.token, value)
        ...
        .catch((err) => {
          switch (err.status) {
            ...
            case 400:
              if (
                err.data.message === "Failed! Phone number is already in use!"
              )
  ...
  const validate = Yup.object({
    name: Yup.string().max(40, "Nhập tối đa 40 ký tự").required("Bắt buộc"),
    email: Yup.string()
          ...
          >
            {(formik) => (
              <Form>
                ...
                <Button variant="secondary" type="reset" disabled={isPosting}>
                  Huỷ
                </Button>
              </Form>
            )}
