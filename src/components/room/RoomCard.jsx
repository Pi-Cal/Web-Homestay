  ...
  const editBtnStyles = useSpring({
    ...
    from: {
      transform: onHover
        ...
        : "translateX(0px) translateY(-25px)",
      scale: onHover ? 0 : 1,
      opacity: onHover ? 0 : 1,
    },
    to: {
    ...
    >
        ...
        <Col sm="12" md="8" className="ps-3 pt-2 pb-2 pe-1 my-card-body">
          <div>
            <Row>
              <Col
                sm="10"
                className="room-title fw-bolder fs-5 overflow-hidden"
                onClick={onClick}
              >
                {room.room_name}
              </Col>
              <Col sm="2">
                {canFavorite && (
                  <FavoriteIcon active={isFavorite} roomId={room.room_id} />
                )}
              </Col>
            </Row>
      ...
      <div className="d-flex justify-content-between my-card-footer mt-1">
        <div>
          ...
          {room.rate !== null
            ? parseFloat(room.rate).toFixed(1) !== 0.0
              ? parseFloat(room.rate).toFixed(1)
              : "Chưa có đánh giá"
            : "Chưa có đánh giá"}
        </div>
        <div>
          <strong>{displayMoney(parseFloat(room.price))}</strong>
          {"/đêm"}
        </div>
      </div>
      {isEditable && (
        <div className="room-edit-option">
          <animated.div style={{ ...editBtnStyles }}>
          ...
          <animated.div style={{ ...removeBtnStyles }}>
            <div
              onClick={() => {
