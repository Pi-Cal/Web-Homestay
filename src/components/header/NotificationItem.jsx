...
const NotificationItem = ({ noti, handleClickNoti }) => {
  ...
  const handleClick = () => {
    ...
    if (noti.status === "UNREAD") {
      handleClickNoti();
      seenNotification(noti.id, userState.token).catch((err) => {
        alert(err);
      });
    }
    if (noti.type === "FEEDBACK") {
      if (location.pathname === `/room/${hiddenNoti.room_id}`)
        window.location.reload();
      else navigate(`/room/${hiddenNoti.room_id}#feedback`);
    } else if (noti.type === "RENTAL") {
      if (hiddenNoti.forHost === true) {
        if (location.pathname === "/rentalmanagement") window.location.reload();
        else navigate(`/rentalmanagement`);
      } else {
        if (location.pathname === `/rental/user/${noti.user_id}`)
          window.location.reload();
