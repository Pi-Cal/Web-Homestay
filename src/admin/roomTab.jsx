...
const RoomRow = ({
    ...
    await deleteRoom(userState.token, room.room_id)
      .then((res) => {
        ...
        socket.emit(
          "send_room",
          room.host_id,
          "Quản trị viên đã xóa phòng của bạn.|null"
        );
      })
    ...
    await updateRoom(userState.token, { ...room, confirmed: true })
      .then((res) => {
        ...
        socket.emit(
          "send_room",
          room.host_id,
          "Quản trị viên đã xác nhận phòng của bạn.|null"
        );
      })
      .catch((err) => alert("System error. Change later"));
    setLoading(false);
  ...
  return (
    <tr>
      ...
      <td className="text-center">
        <Button variant="danger" onClick={handleDeleteRoom} id={room.room_id}>
          Delete
        </Button>
      </td>
    </tr>
...
export const SearchPlaceInput = ({
  ...
  return (
    <Form.Group className={`btn-place ${props.pos ? props.pos : "mb-3"}`}>
        ...
        {({ getInputProps, suggestions, getSuggestionItemProps, loading }) => {
          return (
            <div className="position-relative">
              <Form.Control
                {...getInputProps({
                  className: `input-w100 form-control shadow-none ${
              ...
              {isSearchPlace && (
                <div
                  id="search-place-2"
                  className="gray-border round-radius shadow mt-1"
                >
                  <ListGroup>
      ...
      <ErrorMessage
        name={field.name}
        component="div"
        style={!errStyle ? { color: "red" } : errStyle}
      />
    </Form.Group>
  );
};
...
export const RoomTab = () => {
  ...
  const getAllRoom = async () => {
    ...
    await axios
      .post(
        `${WEB_API}/api/room/admin-search?limit=${roomsPerPage}&page=${currentPage}`,
        {},
        {
          headers: {
  ...
  const getAllRoomWithFilter = () => {
    return axios
      .post(
        `${WEB_API}/api/room/admin-search?limit=${MAX_REQUEST}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${userState.token}`,
  ...
  const getRoomsByPosition = () => {
    ...
    return axios
      .post(`${WEB_API}/api/room/admin-search?limit=${MAX_REQUEST}`, body, {
        headers: {
          Authorization: `Bearer ${userState.token}`,
        },
      })
  ...
  const handleFilterRoom = async () => {
    ...
    const doIt = filterRoom.position
      ? getRoomsByPosition
      : getAllRoomWithFilter;
        ...
        setting
          .then(() => {
            if (filterRoom.host_id) {
              const filterByHostId = res.filter(
                (room) => room.host_id === parseInt(filterRoom.host_id)
              );
          ...
          .then((filterByHostId) => {
            if (filterRoom.room_id) {
              const filterByRoomId = filterByHostId.filter(
                (room) => room.room_id === parseInt(filterRoom.room_id)
              );
    ...
    <Container className="mt-3">
      ...
      <Container>
        ...
        <WePagination
          ...
          isGetting={isLoading}
        />
        <WeToast show={isDeleteToast} onClose={() => setDeleteToast(false)}>
          Delete Success
        </WeToast>
        <WeToast show={isConfirmToast} onClose={() => setConfirmToast(false)}>
          Confirm Success
        </WeToast>
      </Container>
    </Container>
